...
def ortho_xr(ds, GLT_NODATA_VALUE=0, fill_value=-9999):
    ...
    with rasterio.open(out_xr) as rasout:
        rasout.write_crs(ds.spatial_ref, inplace=True)
...
def raw_spatial_crop(ds, shape):
    ...
    with rasterio.open(glt_ds) as rasglt:
        glt_ds.write_crs(glt_ds.spatial_ref, inplace=True)

    # Clip the emit glt
    with rasterio.open(glt_ds) as rasglt:
        clipped = rasglt.clip(shape.geometry.values, shape.crs, all_touched=True)
